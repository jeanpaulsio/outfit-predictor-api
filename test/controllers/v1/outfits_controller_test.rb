module V1
  ...
  class OutfitsControllerTest < ActionDispatch::IntegrationTest
    def setup
      ...
      @auth_headers    = @jerry.create_new_auth_token
    end
    ...
    test 'POST /outfits requires authentication' do
      ...
      assert_match(/You need to sign in/, @response.body)
    end

    test 'POST /outfits on a date without an outfit' do
      assert_difference 'Outfit.count', 1 do
        post v1_outfits_path, headers: @auth_headers, xhr: true, params: {
          outfit: {
            shirt_id: @white_shirt.id,
            pant_id: @denim_pants.id,
            date: '2018-01-03'
